...
| GOV-002 | DONE | `PRODUCT-DECISIONS.md`; scope password reset, search, notification history, PDF, dark mode, deployment, error tracking, dan retention terkunci tanpa ambiguity MVP |
| GOV-003 | DONE | ADR-001 sampai ADR-009 accepted dan direview terhadap PRD serta architecture baseline; ADR-008 tetap local-storage exception |
...
| VIS-001 | DONE | Visible learner preview tersedia di `apps/web`: React 19/Vite reader anemia, page-flip desktop dengan lifecycle DOM terisolasi dan transactional failure cleanup, semantic vertical fallback, keyboard focus/navigation, lazy YouTube block, myth/fact reveal, dan local quiz flow. Frozen install, format, lint 13/13, strict typecheck 13/13, unit/contract tests 22/22, build 13/13, exact license gate (410 records; production 254), production high-severity audit, serta Playwright Edge desktop/mobile 2/2 dengan clean process exit lulus pada 12 Agustus 2026. Ini bukan pengganti VS-009 atau milestone persistence/publishing. |
...
| FND-008 | `packages/observability` | FND-003 | Typed config, structured logging, correlation ID, dan keyed structured-field redaction foundation tersedia | Unit test redaction/correlation; free-form message dilarang memuat secret; tidak ada sensitive log fixture |
